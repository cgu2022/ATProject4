...
# 'bluebutterfly', 'Chris', 'egg',  'Mt. Fuji', 'peppers', 'rainbowpallete', 'Truman', 'X', 'yellowmouse', 'White House'
# '.jpg', '.jpg', '.jpg', '.jpg' ,'.png', '.jpg', '.jpg', '.png', '.jpg', '.jpg'
images = ['GoogleLogoSmall']
imageTypes = ['.jpg']
...
for imgIdx in range(len(images)):
    ...
    for k in kValues:
        ...
        y_pred, centroid_locations = km.predict(img2)
        ...
        print("Written?:", cv2.imwrite(imageName+ "/" + knnType +  ' - ' + 'K=' + str(k) + imageType , res2))
        ...
        res = center[label]
        res2 = res.reshape((img.shape))
        cv2.imwrite(imageName+ "/" + knnType +  ' - ' + 'K=' + str(k) + imageType , res2)