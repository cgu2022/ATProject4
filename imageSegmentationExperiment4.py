...
images = [ 'pepperssmall', 'Xsmall', 'yellowmousesmall', 'Mt. Fujismall', 'GoogleLogoSmaller', 'TrumanSmaller', 'White HouseSmaller']
# 'bluebutterflysmall', 'ChrisSmaller', 'eggsmall',
imageTypes = ['.png', '.png', '.jpg', '.jpg', '.jpg', '.jpg', '.jpg']
# '.jpg', '.jpg', '.jpg',
kValues = list(range(2, 18)) + [50, 100] # 4, 5, 6, 7, 8, 9, 10, 20,
attempts = 100
folderNameEnding = ' attempts- ' + str(attempts) + ' with KMEANS++'
...
for imgIdx in range(len(images)):
    ...
    for k in kValues:
        ...
        y_pred, centroid_locations, times, sse, initTime = km.predict(img2)
        sseList.append(sse)
    ...
    fig = plt.figure()
    finalTotalTimesList = []
    for i in range(0, len(totalTimesList)):
        finalTotalTimesList.append(totalTimesList[i] + initTimes[i])

    plt.plot(kValues, totalTimesList, label = 'Chris TT', \
            color = 'mediumslateblue', lw = 1, marker = 'o', markersize = 3)
    
    plt.plot(kValues, finalTotalTimesList, label = 'Chris TT with Init', \
    ...
    fig = plt.figure()
    plt.plot(kValues, initTimes, label = 'Init Times', \
            color = 'springgreen', lw = 1, marker = 'o', markersize = 3)

    plt.title("Initialization Times plotted across K-values")
    plt.xlabel('K Parameter')
    plt.ylabel("Init Time (seconds)")
    plt.legend(loc="best")

    # plt.show()
    fig.savefig(imageName + folderNameEnding + "/" + 'Init Times Curve Graph.png')