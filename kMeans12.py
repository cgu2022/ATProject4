...
class KMeans:
    ...
    def predict(self, X):
        ...
        random_sample_idxs = np.random.choice(self.n_samples, 1, replace=False)
        self.centroids = [self.X[random_sample_idxs[0]]]
        for i in range(1, self.K):
            distances = distance.cdist(self.X, self.centroids, euclidean_distance)
            minDistances = np.min(distances, axis = 1)
            probs = minDistances / sum(minDistances)
            index = np.random.choice(a = self.n_samples, p = probs)
            #random_sample_idxs.append(index)
            self.centroids.append(self.X[index])

        #self.centroids = [self.X[idx] for idx in random_sample_idxs]
        #print("Initial Centroids:", self.centroids)
        initTime = time.time() - start_time
        ...
        sse = self._calculate_SSE(self.clusters, self.centroids, labels, self.X)
        return labels, self.centroids, times, sse, initTime
    ...
    def _calculate_SSE(self, clusters, centroids, labels, X):
        sse = 0
        for cluster in clusters:
             for sample_idx in cluster:
                #clusters[centroid_idx].append(idx)
                #print(labels[sample_idx])
                sse += euclidean_distance(X[sample_idx], centroids[int(labels[sample_idx])])
        return sse
